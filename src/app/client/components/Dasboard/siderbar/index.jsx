...
import { FiHome, FiMessageSquare, FiSearch, FiVideo, FiBell, FiUser, FiMoreHorizontal } from "react-icons/fi";
  ...
  const NavItem = ({ href, Icon, label }) => {
    ...
    return (
      <li className="mt-3 p-2 rounded-lg">
        <a
          href={href}
          className={`flex flex-col items-center pb-1 transition-all duration-300 ${isActive ? "border-b-2 border-blue-500 text-blue-600 dark:text-blue-300" : "hover:text-blue-600 dark:hover:text-blue-300"
            }`}
        >
      ...
      <div className="bg-gray-100 dark:bg-gray-900 flex-1">
        <nav className="w-full hidden sm:hidden md:flex md:w-40 md:flex-col md:items-center justify-around bg-white dark:bg-gray-800 py-4 fixed bottom-0 md:bottom-auto md:top-0 md:left-0 md:h-screen">
          ...
          <ul className="flex md:flex-col md:mt-2 text-black capitalize space-x-4 md:space-x-0">
            <NavItem href="/client" Icon={FiHome} label="Home" />
            <NavItem href="/client/messages" Icon={FiMessageSquare} label="Mensajes" />
            <NavItem href="/client/search" Icon={FiSearch} label="Buscar" />
            <NavItem href="/client/videos" Icon={FiVideo} label="Videos" />
            <NavItem href="/client/notifications" Icon={FiBell} label="Notificaciones" />
            <NavItem href="/client/profile" Icon={FiUser} label="Perfil" />
          </ul>
